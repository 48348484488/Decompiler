...
void S9xMainLoop (void)
{
	...
	for (;;)
	{
		...
		// --- SNES Deco: Code/Data Logger hook -----------------------------
		// Mirrors the LoROM/HiROM file-offset math used on the Kotlin side
		// (AddressMapper.kt) so offsets logged here line up with the ROM
		// bytes shown in the app's Hex/Disasm tabs.
		{
			uint8_t  cdlBank = Registers.PB;
			uint16_t cdlAddr = Registers.PCw;
			int32_t  cdlOffset = -1;

			if (Memory.HiROM)
			{
				if (!((cdlBank & 0xC0) == 0x00 && cdlAddr < 0x8000))
					cdlOffset = (int32_t) (cdlBank & 0x3F) * 0x10000 + cdlAddr;
			}
			else
			{
				if (cdlAddr >= 0x8000)
			...
			if (cdlOffset >= 0)
			{
				...
				S9xCdlSetCurrentPC(cdlBank, cdlAddr, (uint32_t) cdlOffset);
			}
		}
		// -------------------------------------------------------------------
